...
FLASK_DEBUG = True
HOST = '0.0.0.0' #also change to 127.0.0.1 and see
PORT = 8080
MODEL_DIR = "C:\Users\harih\Desktop\DSS\Env_DSS"
...
print "Creating the decision support system object 'mdss'."
...
def diagnose():
    ...
    if prediction:
      ...
      max_val = random.random()*100
    prediction = [[k, "{0}%".format(round((max_val-v)/float(max_val), 7) * -100)] for k, v in prediction]
    print "Returning predictions:"
    pprint.pprint(prediction)
    for item,value in items:
      prediction.append((item,random.random()*100))
      result= prediction
    return jsonify(result=prediction)  # TODO: Parse the data a bit before returning it, and fix the scores
